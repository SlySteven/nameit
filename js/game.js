...
function updateScoring (button, direction) {
  ...
  console.log(".category-score: ");
    var score_element = $(button).parent().siblings(".category-score").find(".category-score-text");
    ...
    var input = $(button).parent().siblings(".category-input");
    if (score > 0) {
...
function submitAnswers () {
  ...
  for (var i = 0; i < categories.length; i++) {
    ...
    firebase.database().ref('categories/' + categories[i] + '/' + letter + '/' + seed_full + '/' + submissionKey).set(answers[i]);
  }
}
